...
===============================================================================
Created on Wed Dec 3 2025
Author: Marcin Plodzien  (Qilimanjaro Quantum Tech, Barcelona)
...
-------------------------------------------------------------------------------
Framework:      JAX (with automatic differentiation and JIT)
Core Libraries: jax, jax.numpy, optax, matplotlib, numpy
Precision:      64-bit complex arithmetic enabled for stability

Main components:
----------------
- init_purification_params():  Initializes RBM parameters for A(s).
- purification_forward_single():  Computes amplitude-phase neural operator output.
- get_probs_vectorized():  Computes Born probabilities for all measurement bases.
- nll_loss():  Negative log-likelihood objective for training.
- train():  Optimizes parameters using ADAM via jax.value_and_grad.
- get_rho_reconstructed():  Computes rho_est = A @ A_dagger after training.
- compute_detailed_metrics():  Returns fidelity and purity metrics.
