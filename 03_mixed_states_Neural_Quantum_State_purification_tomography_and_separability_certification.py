...
================================================================================
NQO PURIFICATION SCANNER 
================================================================================

The Quantum State Tomography using the Purification Ansatz (rho = A * A^dagger).

PHYSICS CONTEXT:
----------------
1. Purification Ansatz:
   We model a mixed state 'rho' as the partial trace of a pure state |Psi> 
   in a larger Hilbert space (Physical System S + ancilla).
   
       rho = Tr_ancilla [ |Psi><Psi| ] = A @ A^dagger
   
   Here, 'ancilla' is a rectangular matrix of size (2^N, D), where D is the 
   Latent Dimension (Ancilla dimension).
   
   - D=1: Forces a Pure State.
   - D>1: Allows Mixed States (Rank <= D).

2. Neural Parameterization:
   The neural network outputs a vector of amplitudes for a given configuration 's':
       Net(s) -> [a_1, a_2, ..., a_D]
   
   The probability of observing 's' in the computational basis is:
       P(s) = <s|rho|s> = sum_k |A_{s,k}|^2 = || A(s) ||^2

3. Measurement & Rotation (The "Vectorized Trick"):
   To compute probabilities in a rotated basis B (e.g., X basis), we do not
   rotate rho (which is expensive). We rotate the state vectors 'A'.
   
       P(s|B) = || U_B @ A ||^2
   
   We apply U_B using local tensor contractions (tensordot).
"""
