...
================================================================================
NQS ENSEMBLE ENTANGLEMENT SCANNER for Mixed-State Tomography  
================================================================================

This script performs Quantum State Tomography using a Neural Network Ansatz
ensembles for Mixed States.

[Optimized with JAX vmap/jit and Optax for performance, but retaining 
 original file structures and plotting conventions]

PHYSICS CONTEXT:
----------------
1. Pure States vs. Mixed States:
   - A Pure State is described by a state vector |psi>.
   - A Mixed State is described by a Density Matrix 'rho'.
   - Experimental states are always Mixed due to noise.

2. The Ensemble Ansatz:
   rho = sum_{k=1 to K}  w_k * |psi_k><psi_k|,
   where w_k >= 0, sum_k w_k = 1.

3. Entanglement Certification:
   Comparing Unconstrained vs Structured (Partitioned) NQS models.
...
def plot_results_grid(results, cfg):
    ...
    plt.savefig(f"./figures/NQS_Ensemble_Grid_{suffix}.png", dpi=150)
    plt.show()
