...
CONFIG = {
    ...
    # --------------------------------------------------------------
    "epochs": 1200,    # Number of gradient steps with Adam
    "lr": 0.01,       # Learning rate
    "log_every": 10,   # Record loss/fidelity every log_every epochs
...
cfg1["name"] = "Z basis, 5000 shots per basis"
cfg1["measurement_bases"] = ["Z"]   # allowed local axes per qubit            
cfg1["shots_per_basis"] = 5000 # shots per basis
cfg1["shots"] = cfg1["shots_per_basis"] # IMPORTANT: shots and shots_per_basis must be equal   
cfg1["random_bases_per_shot"] = False
CONFIG_LIST.append(cfg1)

cfg2 = copy.deepcopy(CONFIG)
cfg2["name"] = "XYZ basis, 5000 shots per basis"
cfg2["measurement_bases"] = ["X", "Y", "Z"]   # allowed local axes per qubit            
cfg2["shots_per_basis"] = 5000 # shots per basis
cfg2["shots"] = cfg1["shots_per_basis"] # IMPORTANT: shots and shots_per_basis must be equal   
cfg2["random_bases_per_shot"] = False
CONFIG_LIST.append(cfg2)

# Example config: random local XYZ bases with a finite pool
cfg3 = copy.deepcopy(CONFIG)
cfg3["name"] = "Random local XYZ bases, 5000 shots per basis"
cfg3["measurement_bases"] = ["X", "Y", "Z"]   # allowed local axes per qubit            
cfg3["shots_per_basis"] = 5000 # shots per basis
cfg3["shots"] = cfg3["shots_per_basis"] # IMPORTANT: shots and shots_per_basis must be equal   
cfg3["random_bases_per_shot"] = True
cfg3["num_random_bases"] = 3**cfg3["N"]               # random basis strings in the pool
CONFIG_LIST.append(cfg3)
...
def plot_topology_graphs(N: int, ansatzes: List[Dict], cfg: Dict, viz_hidden: int = 12):
    ...
    Subplots are arranged automatically in a roughly square grid.
    """
    ...
    # Scale figure size with grid shape
    fig_width = 4.5 * ncols
    ...
    for ax, ans_cfg in zip(axes_flat, ansatzes):
        name = ans_cfg["name"]
        ...
        for v in range(N):
            for h in range(viz_hidden):
                if mask[v, h] > 0.5:
                    G.add_edge(f"V{v}", f"H{h}")

        # Positions: visibles on bottom, hiddens on top
        pos = {}
        for i in range(N):
            pos[f"V{i}"] = (i * 2.0, 0.0)
        if viz_hidden > 1:
        ...
        for i in range(viz_hidden):
            pos[f"H{i}"] = (i * scale, 1.0)

        nx.draw_networkx_nodes(
            G, pos,
            nodelist=[f"V{i}" for i in range(N)],
            node_color="#1f77b4",
            node_size=150,
            ax=ax,
        )
        nx.draw_networkx_nodes(
            G, pos,
            nodelist=[f"H{i}" for i in range(viz_hidden)],
            node_color="#d62728",
            node_size=50,
            ax=ax,
        )
...
def plot_results_grid(results_loss, results_fid, ansatzes, cfg):
    ...
    styles = {
        ...
        "Product (1|1|1|1|1|1)":   {"c": "#d62728", "ls": ":",  "lw": 3.0},
    }
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.93])
    filename = FIGURES_DIR +  "./fig_nns_snns_fid_{}.png".format(suffix)
